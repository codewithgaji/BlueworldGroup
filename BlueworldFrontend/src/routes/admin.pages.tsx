              ...
              {!isLoading && (data?.length ?? 0) === 0 && (
                <tr>
                  <td colSpan={5} className="px-5 py-10 text-center text-muted-foreground">
                    ...
                    <code className="rounded bg-secondary px-1.5 py-0.5">python -m scripts.seed_pages</code> on the
                    backend to load the four About pages.
                  </td>
                </tr>
              )}
              {(data ?? []).map((page) => (
                  ...
                  <td className="px-5 py-4">
                    <div className="flex justify-end gap-2">
                      <Link
                        ...
                        className="grid h-9 w-9 place-items-center rounded-lg border border-border text-primary-deep transition-colors hover:bg-primary-soft"
                      ...
                      <button
                        ...
                        onClick={() => void handleDelete(page.id, page.slug)}
                        className="grid h-9 w-9 place-items-center rounded-lg border border-border text-destructive transition-colors hover:bg-destructive/10 disabled:opacity-50"
