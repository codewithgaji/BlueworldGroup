...
function Prose({ p }: { p: P }) {
  return (
    <>
      {(p.eyebrow || p.heading) && <SectionHeading eyebrow={p.eyebrow} title={p.heading} />}
...
function ProseImage({ p }: { p: P }) {
  ...
  const text = (
    <div>
      <SectionHeading eyebrow={p.eyebrow} title={p.heading} description={p.description} />
      <div className="mt-6 space-y-5 text-base leading-relaxed text-muted-foreground">
        {(p.paragraphs ?? []).map((t: string, i: number) => <p key={i}>{t}</p>)}
      </div>
    </div>
...
function CardGrid({ p }: { p: P }) {
  const cols = { 2: "sm:grid-cols-2", 3: "sm:grid-cols-2 lg:grid-cols-3", 4: "sm:grid-cols-2 lg:grid-cols-4" }[p.columns ?? 2];
  return (
    <>
      {(p.eyebrow || p.heading) && <SectionHeading eyebrow={p.eyebrow} title={p.heading} />}
...
function LinkCards({ p }: { p: P }) {
  return (
    <>
      {(p.eyebrow || p.heading) && <SectionHeading eyebrow={p.eyebrow} title={p.heading} align="center" />}
...
export function RenderBlock({ block }: { block: PageBlock }) {
  ...
  return (
    <Section tone={block.tone === "muted" ? "muted" : undefined}>
